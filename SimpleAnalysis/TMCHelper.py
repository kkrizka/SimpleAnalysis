...
##                    Radiations are defined as decays where the first daugther has the
...
def final_state_particles(particles,idx,ignoreRadiation=True):
    ...
    for i in range(particle.GetFirstDaughter(),particle.GetFirstDaughter()+particle.GetNDaughters()):
        products.extend(final_state_particles(particles,i))
        if(ignoreRadiation): # Make sure daughter different from parent, if requested
